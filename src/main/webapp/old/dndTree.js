...
//    }
    ...
    /**
     * Compute pixel offset for each depth = 0 for root, 1 for first level children etc.
     * @return a function depth => pixel offset
     */
    function getPixelOffsetFn(nodes) {
      // lenOff[depth] = { max: maxLen, y: pixelOffset }
      // where:
      //   maxLen = length (in chars) of longest label positioned before this level
      //   y = pixel offset for this level
      var lenOff = [];
      function getLenOff(i) {
        return lenOff[i] || (lenOff[i] = { max: 0, y: 0 });
      };
      
      nodes.forEach(function(d) {
        // node with children has label to left of node, it's length counts towards the offset for this depth
        // node without children has label to right of node, it's length counts towards the offset for the next level deeper
        var depth = d.children && d.children.length > 0 ? d.depth : d.depth + 1;
        var lo = getLenOff(depth);
        var l = d.name.length;
        if (l > lo.max) lo.max = l;
      });
      var cummY = 0;
      for (var i = 0; i < lenOff.length; i++) {
        var lo = getLenOff(i);
    ...
    function update(source) {
        ...
        // A node item is a root item augmented with the depth, children (if not already present), parent, x and y properties.
        // The link item just has the source (parent) and target (child) properties. 
        var nodes = tree.nodes(root).reverse(),
            links = tree.links(nodes);
        var pixelOffsetFn = getPixelOffsetFn(nodes);
        nodes.forEach(function(d) {
            d.y = pixelOffsetFn(d.depth);
        });
